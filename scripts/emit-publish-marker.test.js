...
test("emit-publish-marker still works after preview dist persistence runs", (t) => {
  const projectPath = createProject(t, "demo-page", {
    name: "Demo",
    description: "发布说明",
    author: "alice",
  ...
  const persistResult = runPersist(projectPath);
  assert.equal(persistResult.status, 0, persistResult.stderr);
  ...
  assert.equal(
    result.stdout.trim(),
    `##publishEtart##Demo|发布说明|alice|${path.join(projectPath, "dist")}##publishEnd##`
  );
});
