...
在执行任何修复命令前，先用 `/design-taste-frontend` 对页面做一次整体 UI 验收。

**目的**：在进入逐项修复前，用设计视角扫描出视觉层问题，避免修复过程中遗漏主观感知类问题（间距感、层次感、品牌调性等）。
...
**输出处理**：
- 将验收结论中的问题逐条追加到 `.webgen/audit.md`，并标注来源为 `design-taste`
- 按对应 P 级归类后，进入下方修复流水线统一处理
