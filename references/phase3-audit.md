...
- 执行 `Taste Pre-Flight`，复用阶段内已有截图和代码结构，检测设计反模式
- 优先按 Taste 模块选择、三枚旋钮和 Anti-Slop 规则判断，而不是凭主观喜好改视觉
- 若已有截图足够，禁止为 Taste 复查重复截图
- 产出写入 `.webgen/audit.md`，按 P0/P1/P2/P3 分级
- 根据检测结果进行页面优化
...
[ ] 第 1 张：整页或首屏可用性确认
[ ] 第 2 张：关键模块或问题区域复查
[ ] G4_AUDIT → G5_PREVIEW 默认最多 1 张截图
[ ] Taste 审美复查默认复用已有截图，不新增截图
[ ] 只有代码和已有截图仍无法判断时，才允许额外补 1 张局部截图
[ ] 单次页面任务常规总预算 3 张，绝对上限 4 张
```

**达到截图上限后**：

- 停止继续截图
- 基于现有截图、代码结构和 console 信息输出剩余不确定项
- 将无法确认的风险写入 `.webgen/audit.md`
...
运行时验证截图必须计入本阶段截图预算；Taste 复查默认复用该截图，不得单独重拍同一视图。
